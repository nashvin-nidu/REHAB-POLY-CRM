...
export function AppSwitcher() {
    ...
    return (
        <div className="fixed top-0 left-0 right-0 h-12 bg-[#090d13] border-b border-adm-card flex items-center px-4 z-[1000] font-sora">
            <div className="font-lora text-[16px] text-adm-text font-semibold mr-6 tracking-tight flex items-center gap-2">
            ...
            <div className="flex gap-0.5 flex-1">
                ...
                >
                    ...
                    Admin Dashboard
                    <span className="bg-adm-accent text-white rounded-full px-1.5 py-[0.5px] text-[10px] ml-0.5">{patientsCount}</span>
                </Link>
                <Link
                    href="/"
                    className={`px-4 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 transition-all ${!isAdmin ? 'text-adm-text bg-adm-card' : 'text-adm-muted hover:text-adm-text hover:bg-adm-surface'}`}
                >
                    ...
                    <span className="bg-pat-green text-white rounded-full px-1.5 py-[0.5px] text-[10px] ml-0.5">{patient?.firstName} {patient?.lastName?.[0] ?? ''}.</span>
                </Link>
            </div>

            <div className="ml-auto flex items-center gap-2">
                <div className="flex items-center gap-1.5 text-[11px] font-mono text-adm-accent2">
                    <span className="w-1.5 h-1.5 rounded-full bg-adm-accent2 shadow-[0_0_4px_rgba(63,185,80,0.4)] animate-pulse" />
                    Zustand · Live
                </div>
                <div className="text-[11px] font-mono text-adm-muted min-w-[46px] text-right">
                    {time}
                </div>
            </div>
        </div>
