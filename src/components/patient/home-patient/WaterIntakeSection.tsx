...
export function WaterIntakeSection() {
    const glasses = [1, 2, 3, 4, 5, 6, 7, 8];
    const currentIntake = 4;
        ...
        <Section title="Water Intake" action={`${currentIntake}/${glasses.length} glasses`}>
            <div className="bg-pat-card rounded-2xl p-4 border border-pat-border shadow-sm mb-3 lg:mb-5">
                <div className="flex justify-between gap-1.5">
                    {glasses.map(i => (
                        <div key={i} className={`flex-1 aspect-[1/1.3] rounded-t-lg rounded-b-md border-[1.5px] cursor-pointer transition-all hover:-translate-y-px ${i <= currentIntake ? 'bg-blue-100 border-blue-400/40 relative overflow-hidden shadow-sm' : 'bg-pat-bg border-pat-border border-dashed'}`}>
