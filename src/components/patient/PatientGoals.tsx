    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-pat-bg font-sora pb-6 relative">
            <div className="bg-gradient-to-br from-[#1c3557] to-[#1e4a7a] px-5.5 py-4 shrink-0 lg:sticky lg:top-0 lg:z-50 shadow-md">
                <div className="text-[10px] text-white/50 font-medium tracking-[0.5px] uppercase mb-0.5">Recovery Journey</div>
                <div className="text-[20px] font-extrabold text-white tracking-tight leading-tight">Goals & Milestones</div>
                <div className="text-[11px] text-white/60 mt-1">4 of 7 achieved</div>
            </div>
                ...
                <div className="lg:grid lg:grid-cols-2 lg:gap-8 lg:items-start w-full">
                    <div className="flex flex-col">
                        <Section title="Weekly Streak">
                            <div className="bg-pat-card rounded-[20px] p-4 border border-pat-border shadow-sm mb-4 lg:mb-6">
                                <div className="flex justify-between items-center mb-3">
                                    <div className="text-[24px] font-extrabold text-pat-text leading-none">14 <span className="text-[12px] font-medium text-pat-muted">days</span></div>
                                    <div className="text-[10px] font-bold text-[#d97706] bg-[#fef3c7] px-2 py-1 rounded-md">🔥 On Fire!</div>
                                </div>
                                <div className="flex justify-between gap-1">
                                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                                        <div key={i} className="flex flex-col items-center gap-1.5">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold transition-all ${i < 5 ? 'bg-pat-blue text-white shadow-[0_2px_8px_rgba(37,99,235,0.3)]' : i === 5 ? 'bg-pat-blue-soft border-2 border-pat-blue text-pat-blue scale-110' : 'bg-pat-bg border border-pat-border text-pat-muted'}`}>{i < 5 ? '✓' : ''}</div>
                                            <div className={`text-[10px] font-bold ${i === 5 ? 'text-pat-blue' : 'text-pat-muted'}`}>{d}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <Section title="Milestones">
                            <div className="bg-pat-card rounded-[20px] border border-pat-border shadow-sm mb-4 lg:mb-6 overflow-hidden">
                                {[
                                    ...
                                    <div key={i} className={`flex items-start gap-3 p-3.5 border-b border-pat-border last:border-0 ${m.done ? 'opacity-70' : ''}`}>
                                        ...
                                        <div>
                                            <div className={`text-[12px] font-bold mb-0.5 ${m.done ? 'text-pat-text line-through decoration-pat-muted/50' : 'text-pat-text'}`}>{m.title}</div>
                                            <div className="text-[10px] text-pat-muted leading-tight">{m.desc}</div>
                                            <div className={`text-[9px] font-bold uppercase tracking-wide mt-1.5 ${m.done ? 'text-pat-muted' : 'text-pat-blue'}`}>{m.date}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="flex flex-col mt-4 lg:mt-0">
                        <Section title="Recovery Progress">
                            <div className="bg-pat-card rounded-[20px] p-4 border border-pat-border shadow-sm mb-4 lg:mb-6 space-y-3.5">
                        ...
                        <Section title="Recovery Chart">
                            <div className="bg-pat-card rounded-[20px] p-4 border border-pat-border shadow-sm h-[200px] lg:h-[240px]">
...
function Section({ title, children }: any) {
    return (
        <>
            <div className="text-[10px] font-bold uppercase tracking-[1px] text-pat-muted mb-2 px-1">
                {title}
            </div>
            {children}
        </>
    ...
    return (
        <div>
            <div className="flex justify-between mb-1.5">
                <span className="text-[11px] font-bold text-pat-text">{label}</span>
