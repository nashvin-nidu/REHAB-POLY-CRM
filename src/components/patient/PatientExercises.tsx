                ...
                <div className="flex overflow-x-auto bg-[#1c3557] px-3.5 pt-2.5 scrollbar-hide relative z-40 border-b border-[#1e4a7a]">
                    {levels.map(l => (
                        <button key={l} onClick={() => setLevel(l)} className={`px-3.5 py-2 rounded-t-lg text-11 font-bold whitespace-nowrap transition-all ${level === l ? 'bg-pat-bg text-pat-navy' : 'text-white/50 bg-transparent hover:text-white'}`}>
                            {l}
                            {l !== 'All Exercises' && <span className={`inline-block w-4 h-4 ml-1 rounded-full text-center leading-4 text-9 font-extrabold ${l === 'Beginner' ? 'bg-[#dcfce7] text-[#16a34a]' : l === 'Intermediate' ? 'bg-[#fef3c7] text-[#d97706]' : 'bg-[#fee2e2] text-[#dc2626]'}`}>{exercises.filter(e => e.difficulty === l).length}</span>}
                        </button>
                    ))}
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 lg:gap-6 w-full">
                    {filteredExercises.map(ex => (
                        <div key={ex.id} className="bg-pat-card border-[1.5px] border-pat-border rounded-2xl overflow-hidden shadow-sm hover:-translate-y-0.5 transition-all cursor-pointer" onClick={() => setActiveVideo(ex.id)}>
                            <div className="h-35 md:h-180px bg-gradient-to-br from-[#1c3557] to-[#1e4a7a] relative flex items-center justify-center group">
                                ...
                                <div className="absolute top-2.5 left-2.5 bg-black/60 text-white text-10 font-bold px-2 py-0.5 rounded-md uppercase tracking-[0.3px]">{ex.category.split('—')[0]}</div>
                                <div className="absolute bottom-2.5 right-2.5 bg-black/70 text-white text-10 font-semibold px-7px py-0.5 rounded font-mono">{ex.duration}:00</div>
                            </div>
                            <div className="p-3 lg:p-4">
                                <div className="text-13 lg:text-14 font-extrabold text-pat-text mb-3px">{ex.name}</div>
                                <div className="text-11 lg:text-12 text-pat-muted truncate max-w-full mb-2" title={ex.instructions}>{ex.instructions}</div>
                                <div className="flex gap-1.5 items-center mt-3">
                                    <span className="text-9 lg:text-10 font-bold uppercase py-1 px-2.5 bg-pat-bg rounded-md text-pat-muted tracking-[0.3px] border border-pat-border">{ex.target.split(',')[0]}</span>
                                    <button className="ml-auto bg-pat-blue text-white font-bold text-11 px-4 py-2 rounded-10px hover:scale-105 transition-transform" onClick={(e) => { e.stopPropagation(); setActiveVideo(ex.id); }}>Start Exercise</button>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-[2px] animate-in fade-in">
                    <div className="bg-pat-card w-450px max-w-full rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 font-sora relative z-[101]">
                        <div className="h-250px bg-[#0d1117] relative flex flex-col items-center justify-center gap-3">
                            ...
                            <div className="text-white text-14 font-bold z-10 text-center px-4">{exercises.find(e => e.id === activeVideo)?.name}</div>
                        </div>
                        <div className="p-5">
